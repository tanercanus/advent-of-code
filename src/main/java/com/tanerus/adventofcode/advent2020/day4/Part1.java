...
public class Part1 {
    ...
    public static long validPasswordsCount(List<Passport> passportList) {
        return passportList.stream().filter(
                x -> (x.getByr() != null && x.getEcl() != null && x.getEyr() != null &&
                        x.getHcl() != null && x.getHgt() != null && x.getIyr() != null && x.getPid() != null)
        ).count();
    }
    ...
    public static List<Passport> createAllPassport(String[] inputArray, boolean addExtraValidation) {
        ...
        while (inputArray.length > index) {
            ...
            boolean isPassportOkay = false;
            boolean isIncreasedIndex = false;
            while (!isPassportOkay) {
                if (endIndex != inputArray.length && !"".equals(inputArray[endIndex])) {
                    endIndex++;
                    isIncreasedIndex = true;
                } else {
                    isPassportOkay = true;
                }
            }
